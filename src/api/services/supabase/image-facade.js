...
export const getImage = async ( username ) => {
  try {
    const response = await axios.get(`${IMAGE_BASE_URL}/${username}/avatar.png`)
...
export const getImagesForUsers = async ( usernames ) => {
  ...
  for (const username of usernames) {
    ...
    if (image === null) {
      usernamesWithImages.push({"username": username, "imageurl": "https://www.pngitem.com/pimgs/m/30-307416_profile-icon-png-image-free-download-searchpng-employee.png"})
      continue
    } 
    else {
